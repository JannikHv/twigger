...
const readTwiggerConfig = () => {
  ...
  if (fs.existsSync(configPathYaml)) {
    console.log(`${nanocolors.yellow('[~]')} Using ${nanocolors.bold('twigger-config.yaml')}`);
    userConfig = yaml.parse(fs.readFileSync(configPathYaml, 'utf-8'));
  } else if (fs.existsSync(configPathJson)) {
    console.log(`${nanocolors.yellow('[~]')} Using ${nanocolors.bold('twigger-config.json')}`);
    userConfig = require(configPathJson)
  } else {
    console.log(`${nanocolors.yellow('[~]')} No twigger-config found`);
  }

  return { ...defaultTwiggerConfig, ...userConfig };
};
